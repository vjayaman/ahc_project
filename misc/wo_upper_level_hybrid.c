...
static void manager(double *A) {
    ...
    end_time = clock();
    double time_interval = (double) (end_time - start_time) / CLOCKS_PER_SEC;
...
static void sequential(double *A) {    
    ...
    //double *Dists = (double *)malloc(NUM_VECS * NUM_VECS * sizeof(double));    
    resetMatrix(NUM_VECS, NUM_VECS, Dists);
    ...
    printf("\n\nMinimum distance %.3f found at (%d, %d), level %d\n\tFrom [%.2f, %.2f] and [%.2f, %.2f] to [%.2f, %.2f]\n", 
               coords[2], val0, val1, NUM_VECS,
               *(A + val0*N + 0), *(A + val0*N + 1), *(A + val1*N + 0), *(A + val1*N + 1), new_vec[0], new_vec[1]);
    printf("\n--------------------------------------------\n\n");
