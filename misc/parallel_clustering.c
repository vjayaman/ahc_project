...
//           $ gcc -Wall seq_clustering.c -o seq_clustering -lm
...
static void manager() {    
    ...
    // N = 4, i goes from 0 to NUM_VECS-1
    double x_i[N];
    double A[NUM_VECS * N]; 
    int i, j;
    int num_procs;
    ...
    double merged_pairs[NUM_VECS * 4];
...
static void worker(int chunk_size) {
    ...
    for (;;) {
        ...
        double merged_pairs[chunk_size * 4];
        ...
        printf("\nMinimum distance %.3f found at (%d, %d)\n\tFor [%.2f, %.2f] and [%.2f, %.2f]\n", 
            coords[2], val0, val1, 
            *(A + val0*N + 0), *(A + val0*N + 1), 
            *(A + val1*N + 0), *(A + val1*N + 1));
        printf("\n--------------------------------------------\n\n");

        addToMergedList(merged_pairs, A, val0, val1, chunk_size - 1);
